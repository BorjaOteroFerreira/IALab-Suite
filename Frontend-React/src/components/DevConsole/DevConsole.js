...
const DevConsole = () => {
    ...
    const openPictureInPicture = async () => {
        ...
        try {
            ...
            const handlePipClose = () => {
                setPipWindow(null);
                setIsPiPMode(false);
                // Asegurar que la consola vuelve a ser visible en la app principal
                // No cambiar isVisible aquí, mantener el estado
            };
    ...
    const closePictureInPicture = () => {
        ...
        setPipWindow(null);
        setIsPiPMode(false);
        // La consola debe seguir visible en la app principal
        // No cambiar isVisible aquí
    };
    ...
    // No renderizar nada si no es visible y no está en modo PiP
    if (!isVisible && !isPiPMode) return null;
    ...
    if (isPiPMode && pipWindow && !pipWindow.closed) {
        ...
        if (pipContainer) {
            return createPortal(<ConsoleContent />, pipContainer);
        }
    }
